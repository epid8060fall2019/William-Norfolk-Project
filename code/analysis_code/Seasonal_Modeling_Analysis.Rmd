...
pltree(cluster_site, cex = 0.6, hang = -1, main = "Dendrogram of Sample Locations", labels = cluster_ready$location)
```

```{r}
sub_group_dendro_1 <- cutree(cluster_site, k = 4)
...
fviz_cluster(list(data = cluster_site, cluster = sub_group_dendro_1))
...
```{r}
cuttest <- cutree(xtest, 3)
#table(cuttest$cluster, remove_nas$season)
...
```{r}
set.seed(20)
ytest <- kmeans(remove_nas[, 2:6], 3, nstart = 20)
ytest
```

```{r}
table(ytest$cluster, remove_nas$site_type)
